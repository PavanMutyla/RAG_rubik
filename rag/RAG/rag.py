...
def classify_query(state, chain=query_classifier):
    ...
    return {'data': [], 'query': query, 'relevant_categories': relevant_categories, 'web_search': 'yes', 'user_data': user_data , 'allocations':state['allocations']}
...
def fetch_data(state, vector_dbs = vector_dbs, tailvy_api_client=web_search_tool) -> dict:
    ...
    return {'data': results, 'query':query,'relevant_categories':relevant_categories, 'web_search':'yes', 'user_data': user_data, 'allocations':state['allocations']}
...
def grade_data(state):
    ...
    for data in docs:
        ...
        if grade == 'yes':
            ...
            web = 'yes'
    return {"documents": filterd_data, "question": question, "web_search": web, 'user_data': user_data, 'allocations':state['allocations']}
...
def web_search(state,tailvy_api_client=web_search_tool ):
    ...
    data.extend(docs)
    return {'data':data, 'query':query,  'relevant_categories':state['relevant_categories'], 'user_data': user_data,  'allocations':state['allocations']}
